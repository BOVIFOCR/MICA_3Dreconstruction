...
def load_all_neighboor_images(img_path, level=0, limit=-1):
    ...
    neighboor_paths = sorted(glob(path_dir_img + '/*' + img_ext))
...
def main(args):
    ...
    if args.l == True:
        level=0     # search only on current folder
        limit = -1  # load all neighboors
        print('Loading neighboor files...')
        neighboor_paths = load_all_neighboor_images(args.i, level, limit)
