...
class MLFW_3DMM(torch.utils.data.Dataset):
    ...
    def _class_name_from_path_sample(self, path=''):
        class_name = '_'.join(path.split('/')[-2].split('_')[:-3])
        return class_name

    def load_paths(self, root_dir='', file_ext=''):
        paths = sorted(glob(root_dir + '/*/*/*'+file_ext))     # MLFW is organized as: /path/to/MLFW/origin/Aaron_Eckhart_0001_0000/identity.npy
        # print('LFW_3DMM - load_paths - paths:', paths); sys.exit(0)
        return paths
